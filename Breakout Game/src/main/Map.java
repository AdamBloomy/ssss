			...
			for (int row = 0; row < theMap.length; row++) {
				for (int col = 0; col < theMap[0].length; col++) {
					if (theMap[row][col] > 0) {
						...
						g.fillRoundRect(col * brickWidth + HOR_PAD + BETWEEN_PAD * col,
								row * brickHeight + VERT_PAD + BETWEEN_PAD * row, brickWidth, brickHeight, 10, 10);
						g.setStroke(new BasicStroke(2));
						// g.setColor(GamePanel.BGCOLOR);
						// g.drawRect(col * brickWidth + HOR_PAD, row * brickHeight + VERT_PAD,
