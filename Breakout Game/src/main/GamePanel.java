...
public class GamePanel extends JPanel {
	...
	public void init() {
		theHud = new HUD();
		mousex = 0;
		woodNoise = "file:./Resources/woodhit.aif";
		...
		for (int row = 0; row < theMap.getMapArray().length; row++) {
			for (int col = 0; col < theMap.getMapArray()[0].length; col++) {
				...
				if (brick > 0) {
					...
					ArrayList<Rocket> rs = thePaddle.getGun().getRockets();

					for (int j = 0; j < rs.size(); j++) {

						Rectangle rr = rs.get(j).getRocketRect1();

						if (rr.intersects(brickRect)) {
							...
							playSound("file:./Resources/bomb.aif", 0);
							rs.get(j).setIsActive(false);
							theHud.addScore(50);
						}
					}
				}
		...
		for (int i = 0; i < powerUps.size(); i++) {
			...
			if (paddleRect.intersects(puRect)) {
				...
				if (powerUps.get(i).getType() == PowerUp.EXTRABALL && !powerUps.get(i).getWasUsed()) {
					...
					theExtraBall = new Ball((int) theBall.getX(), theBall.getY() + 1, dx,
							(int) theBall.getDY() / 2, theBall.getSize(), theHud);
					powerUps.get(i).setWasUsed(true);
